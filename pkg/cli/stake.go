...
func (self *DepositCmd) Run(cli *CLI, ctx context.Context, logger log.Logger) error {
	...
	opts, err := ethereum.PrepareEthTransaction(ctx, client, account, self.GasMaxFee, contracts.DepositStakeGasLimit)
	if err != nil {
...
func (self *WithdrawCmd) Run(cli *CLI, ctx context.Context, logger log.Logger) error {
	...
	opts, err := ethereum.PrepareEthTransaction(ctx, client, account, self.GasMaxFee, contracts.WithdrawStakeGasLimit)
	if err != nil {
		return errors.Wrap(err, "prepare ethereum transaction")
	}
...
func (self *RequestCmd) Run(cli *CLI, ctx context.Context, logger log.Logger) error {
	...
	opts, err := ethereum.PrepareEthTransaction(ctx, client, account, self.GasMaxFee, contracts.RequestStakingWithdrawGasLimit)
	if err != nil {
		return errors.Wrap(err, "prepare ethereum transaction")
	}
